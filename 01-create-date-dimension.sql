--IMPORTANT - After Creation DateDimension table - change the date type of the Date (PK) column to DateTime

Use <Insert_Your_Database_Name>;

DECLARE @StartDate DATE = '19600101', @NumberOfYears INT = 70;
...
(
  --DateKey           INT         NOT NULL PRIMARY KEY, -- Alternative if INT to be used as PK
  [Date]              DATE        NOT NULL PRIMARY KEY, -- Sets DATE as PK
...
UPDATE x SET IsHoliday = 1, HolidayText = CASE
  ...
  WHEN ([MonthName] = 'December' AND [Day] = 25)
    THEN 'Christmas Day'
  WHEN ([MonthName] = 'January' AND [Day] = 25)
    THEN 'Burns Night'
  WHEN ([MonthName] = 'December' AND [Day] = 26)
    THEN 'Boxing Day'
  WHEN ([MonthName] = 'December' AND [Day] = 31)
    THEN 'Hogmanay'
  WHEN ([MonthName] = 'November' AND [Day] = 30)
    THEN 'St Andrews Day'
  END
WHERE 
  ([Date] = FirstDayOfYear)
